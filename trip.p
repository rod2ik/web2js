...
{:6}const{11:}mem_max=3000;mem_min=1;buf_size=500;error_line=64;
half_error_line=32;max_print_line=72;stack_size=200;max_in_open=6;
font_max=75;font_mem_size=20000;param_size=60;nest_size=40;
max_strings=3000;string_vacancies=8000;pool_size=33764;save_size=600;
...
cur_list.head_field:=2999;cur_list.tail_field:=2999;
cur_list.aux_field.int:=-65536000;cur_list.ml_field:=0;
cur_list.pg_field:=0;shown_mode:=0;{991:}page_contents:=0;
page_tail:=2998;mem[2998].hh.rh:=0;last_glue:=65535;last_penalty:=0;
...
{:1343}{164:}for k:=2 to 20 do mem[k].int:=0;k:=1;
...
mem[lo_mem_max].hh.lh:=0;for k:=2987 to 3000 do mem[k]:=mem[lo_mem_max];
{790:}mem[2990].hh.lh:=6714;{:790}{797:}mem[2991].hh.rh:=256;
mem[2991].hh.lh:=0;{:797}{820:}mem[2993].hh.b0:=1;
mem[2994].hh.lh:=65535;mem[2993].hh.b1:=0;
{:820}{981:}mem[3000].hh.b1:=255;mem[3000].hh.b0:=1;
mem[3000].hh.rh:=3000;{:981}{988:}mem[2998].hh.b0:=10;
mem[2998].hh.b1:=0;{:988};avail:=0;mem_end:=3000;hi_mem_min:=2987;
var_used:=20;dyn_used:=14;{:164}{222:}eqtb[2881].hh.b0:=101;
eqtb[2881].hh.rh:=0;eqtb[2881].hh.b1:=0;
for k:=1 to 2880 do eqtb[k]:=eqtb[2881];{:222}{228:}eqtb[2882].hh.rh:=1;
eqtb[2882].hh.b1:=1;eqtb[2882].hh.b0:=117;
for k:=2883 to 3411 do eqtb[k]:=eqtb[2882];
mem[1].hh.rh:=mem[1].hh.rh+530;{:228}{232:}eqtb[3412].hh.rh:=0;
...
{:951}{1216:}hash[2614].rh:=1190;{:1216}{1301:}format_ident:=1257;
...
help_line[0]:=280;end;show_context;goto 22;end{:88};68:begin debug_help;
goto 22;end;
...
3:begin print(571);p:=2997;end;4:begin print(572);p:=2996;end;
...
if lo_mem_max+2<hi_mem_min then if lo_mem_max+2<=65536 then{126:}begin
...
if t>65536 then t:=65536;mem[q+1].hh.rh:=rover;mem[q+1].hh.lh:=p;
...
{933:}for q:=0 to 307 do begin if hyph_list[q]=p then begin print_nl(940
...
2:print_char(124);10:if mem[p+1].hh.lh<>1 then print_char(32);
...
if p=0 then begin{986:}if 2998<>page_tail then begin print_nl(980);
if output_active then print(981);show_box(mem[2998].hh.rh);
if page_contents>0 then begin print_nl(982);print_totals;print_nl(983);
print_scaled(page_so_far[0]);r:=mem[3000].hh.rh;
while r<>3000 do begin print_ln;print_esc(330);t:=mem[r].hh.b1-0;
...
if mem[r].hh.b0=1 then begin q:=2998;t:=0;repeat q:=mem[q].hh.rh;
...
if mem[2999].hh.rh<>0 then print_nl(368);end;
...
eqtb[5283].int:=sys_time;eqtb[5284].int:=sys_day;
eqtb[5285].int:=sys_month;eqtb[5286].int:=sys_year;end;
...
{:1278}{1287:}57:if chr_code=4239 then print_esc(1237)else print_esc(
...
if cur_cmd=63 then begin_token_list(2990,2)else begin_token_list(mem[
...
repeat mem[2997].hh.rh:=0;
if(mem[r].hh.lh>3583)or(mem[r].hh.lh<3328)then s:=0 else begin match_chr
:=mem[r].hh.lh-3328;s:=mem[r].hh.rh;r:=s;p:=2997;m:=0;end;
{392:}22:get_token;
...
end;back_error;end;pstack[n]:=mem[2997].hh.rh;
...
dyn_used:=dyn_used+1;end;end;mem[p].hh.rh:=q;mem[q].hh.lh:=cur_tok;p:=q;
end;get_token;
if cur_tok=par_token then if long_state<>112 then{396:}begin if
long_state=111 then begin runaway;begin if interaction=3 then;
print_nl(262);print(645);end;sprint_cs(warning_index);print(646);
...
dyn_used:=dyn_used-1;end;p:=mem[2997].hh.rh;pstack[n]:=mem[p].hh.rh;
begin mem[p].hh.rh:=avail;avail:=p;dyn_used:=dyn_used-1;end;
end else pstack[n]:=mem[2997].hh.rh;n:=n+1;
...
co_backup:=cur_order;backup_backup:=mem[2987].hh.rh;
...
radix:=radix_backup;cur_order:=co_backup;mem[2987].hh.rh:=backup_backup;
end;{:366}{380:}procedure get_x_token;label 20,30;begin 20:get_next;
...
var p:halfword;q:halfword;k:pool_pointer;begin p:=2987;mem[p].hh.rh:=0;
...
k:=k+1;end else if(cur_cmd<>10)or(p<>2987)then begin back_input;
if p<>2987 then begin_token_list(mem[2987].hh.rh,3);scan_keyword:=false;
goto 10;end;end;flush_list(mem[2987].hh.rh);scan_keyword:=true;10:end;
{:407}{408:}procedure mu_error;begin begin if interaction=3 then;
print_nl(262);print(662);end;begin help_ptr:=1;help_line[0]:=663;end;
error;end;{:408}{409:}procedure scan_int;forward;
...
print_nl(262);print(802);end;print_esc(hash[2624+f].rh);print(820);
...
end else begin if cur_chr=2 then cur_val:=1 else cur_val:=0;
...
p:=mem[p].hh.rh;begin mem[q].hh.rh:=avail;avail:=q;dyn_used:=dyn_used-1;
end;end;f:=round_decimals(k);if cur_cmd<>10 then back_input;end{:452};
end;end;if cur_val<0 then begin negative:=not negative;
cur_val:=-cur_val;end;
...
if negative then cur_val:=-cur_val;end;{462:}q:=new_spec(1);
mem[q+1].int:=cur_val;
...
init_pool_ptr);end;p:=2997;mem[p].hh.rh:=0;k:=b;
...
if cur_val_level>=4 then{466:}begin p:=2997;mem[p].hh.rh:=0;
...
if q=0 then q:=get_avail else begin avail:=mem[q].hh.rh;mem[q].hh.rh:=0;
dyn_used:=dyn_used+1;end;end;mem[p].hh.rh:=q;mem[q].hh.lh:=mem[r].hh.lh;
p:=q;end;r:=mem[r].hh.rh;end;end;the_toks:=p;
...
{:465}{467:}procedure ins_the_toks;begin mem[2988].hh.rh:=the_toks;
begin_token_list(mem[2997].hh.rh,4);end;{:467}{470:}procedure conv_toks;
var old_setting:0..21;c:0..5;save_scanner_status:small_number;
b:pool_pointer;begin c:=cur_chr;{471:}case c of 0,1:scan_int;
...
end{:472};selector:=old_setting;mem[2988].hh.rh:=str_toks(b);
begin_token_list(mem[2997].hh.rh,4);end;
...
end;end;32:x_token end{:478}else get_token;
...
if eqtb[5299].int>1 then begin begin_diagnostic;print(783);print_int(n);
...
begin if eqtb[5298].int>0 then begin begin_diagnostic;print_nl(825);
...
help_line[0]:=1012;end;error;repeat get_token;until cur_tok=6717;
...
6:{652:}begin mem[2988]:=mem[p+1];mem[2988].hh.rh:=mem[p].hh.rh;p:=2988;
goto 21;end{:652};others:end;goto 15;
...
begin if eqtb[5297].int>0 then begin print_nl(338);print_ln;print(829);
end;
...
if eqtb[5297].int<=0 then begin begin_diagnostic;print_nl(836);
...
old_setting:=selector;selector:=21;print(827);print_int(eqtb[5286].int);
...
{639:}if eqtb[5294].int>1 then begin print_nl(830);print_int(var_used);
print_char(38);print_int(dyn_used);print_char(59);end;
flush_node_list(p);if eqtb[5294].int>1 then begin print(831);
...
mem[mem[q].hh.rh+1].int:=eqtb[5846].int;end;print_ln;print_nl(852);
print_scaled(-x-total_shrink[0]);print(853);goto 50;end{:666};
end else if o=0 then if mem[r+5].hh.rh<>0 then{667:}begin last_badness:=
badness(-x,total_shrink[0]);
if last_badness>eqtb[5289].int then begin print_ln;print_nl(854);
...
begin print_ln;print_nl(857);print_scaled(-x-total_shrink[0]);
print(858);goto 50;end{:677};
end else if o=0 then if mem[r+5].hh.rh<>0 then{678:}begin last_badness:=
badness(-x,total_shrink[0]);
if last_badness>eqtb[5290].int then begin print_ln;print_nl(859);
...
free_node(b,7);b:=new_glue(13);mem[b].hh.rh:=p;
while mem[p].hh.rh<>0 do p:=mem[p].hh.rh;mem[p].hh.rh:=new_glue(13);
...
mlist_to_hlist;q:=mem[2997].hh.rh;cur_style:=save_style;
...
cur_style-2)div 2);
cur_mu:=x_over_n(font_info[6+param_base[eqtb[3937+cur_size].hh.rh]].int,
18);end{:703};p:=hpack(mem[2997].hh.rh,0,1);end;
...
{729:}if r_type=18 then mem[r].hh.b0:=16{:729};{760:}p:=2997;
...
mem[p].hh.lh:=cur_align;mem[p+1].hh.lh:=mem[2992].hh.rh;
...
mem[2992].hh.rh:=mem[p+1].hh.lh;cur_align:=mem[p].hh.lh;
...
mode_field{:775};scan_spec(6,false);{777:}mem[2992].hh.rh:=0;
cur_align:=2992;cur_loop:=0;scanner_status:=4;
...
{779:}{783:}p:=2996;mem[p].hh.rh:=0;
while true do begin get_preamble_token;if cur_cmd=6 then goto 31;
if(cur_cmd<=5)and(cur_cmd>=4)and(align_state=-1000000)then if(p=2996)and
(cur_loop=0)and(cur_cmd=4)then cur_loop:=cur_align else begin begin if
interaction=3 then;print_nl(262);print(903);end;begin help_ptr:=3;
help_line[2]:=904;help_line[1]:=905;help_line[0]:=906;end;back_error;
goto 31;
end else if(cur_cmd<>10)or(p<>2996)then begin mem[p].hh.rh:=get_avail;
p:=mem[p].hh.rh;mem[p].hh.lh:=cur_tok;end;end;31:{:783};
mem[cur_align].hh.rh:=new_null_box;cur_align:=mem[cur_align].hh.rh;
mem[cur_align].hh.lh:=2991;mem[cur_align+1].int:=-1073741824;
mem[cur_align+3].int:=mem[2996].hh.rh;{784:}p:=2996;mem[p].hh.rh:=0;
...
mem[cur_align+2].int:=mem[2996].hh.rh{:779};end;
...
begin mem[cur_list.tail_field].hh.rh:=new_glue(mem[mem[2992].hh.rh+1].hh
.lh);cur_list.tail_field:=mem[cur_list.tail_field].hh.rh;end;
mem[cur_list.tail_field].hh.b1:=12;
cur_align:=mem[mem[2992].hh.rh].hh.rh;cur_tail:=cur_head;
...
mem[p].hh.lh:=2991;mem[p+1].int:=-1073741824;
cur_loop:=mem[cur_loop].hh.rh;{794:}q:=2996;r:=mem[cur_loop+3].int;
while r<>0 do begin mem[q].hh.rh:=get_avail;q:=mem[q].hh.rh;
mem[q].hh.lh:=mem[r].hh.lh;r:=mem[r].hh.rh;end;mem[q].hh.rh:=0;
mem[p+3].int:=mem[2996].hh.rh;q:=2996;r:=mem[cur_loop+2].int;
while r<>0 do begin mem[q].hh.rh:=get_avail;q:=mem[q].hh.rh;
mem[q].hh.lh:=mem[r].hh.lh;r:=mem[r].hh.rh;end;mem[q].hh.rh:=0;
mem[p+2].int:=mem[2996].hh.rh{:794};cur_loop:=mem[cur_loop].hh.rh;
...
{801:}q:=mem[mem[2992].hh.rh].hh.rh;repeat flush_list(mem[q+3].int);
...
if s<>1 then begin mem[1].hh.rh:=mem[1].hh.rh+1;delete_glue_ref(s);
mem[r+1].hh.lh:=1;end;end{:802};
if mem[q].hh.lh<>2991 then{803:}begin t:=mem[q+1].int+mem[mem[mem[q].hh.
rh+1].hh.lh+1].int;r:=mem[q].hh.lh;s:=2991;mem[s].hh.lh:=p;n:=1;
...
hh.lh+1].int:=mem[r+1].int;free_node(r,2);end;r:=u;until r=2991;
...
eqtb[5846].int:=0;
p:=hpack(mem[2992].hh.rh,save_stack[save_ptr+1].int,save_stack[save_ptr
...
p:=vpackage(mem[2992].hh.rh,save_stack[save_ptr+1].int,save_stack[
save_ptr+0].int,1073741823);q:=mem[mem[2992].hh.rh].hh.rh;
...
repeat{808:}n:=mem[r].hh.b1;t:=mem[s+1].int;w:=t;u:=2996;
...
s:=mem[mem[s].hh.rh].hh.rh;until r=0;
...
if eqtb[5295].int>0 then end_diagnostic(true);
...
if eqtb[5295].int>0 then begin_diagnostic;end;q:=new_spec(p);
...
-10000{:831};no_break_yet:=true;prev_r:=2993;old_l:=0;
...
easy_line)or(r=2993))then{836:}begin if no_break_yet then{837:}begin
...
end else if prev_r=2993 then begin active_width[1]:=break_width[1];
...
if eqtb[5295].int>0 then{846:}begin print_nl(925);
...
minimum_demerits:=1073741823;{844:}if r<>2993 then begin q:=get_node(7);
mem[q].hh.rh:=r;mem[q].hh.b0:=2;mem[q].hh.b1:=0;
...
minimum_demerits=1073741823)and(mem[r].hh.rh=2993)and(prev_r=2993)then
...
if prev_r=2993 then{861:}begin r:=mem[2993].hh.rh;
...
cur_active_width[4]:=active_width[4];
cur_active_width[5]:=active_width[5];
cur_active_width[6]:=active_width[6];mem[2993].hh.rh:=mem[r].hh.rh;
free_node(r,7);end;
end{:861}else if mem[prev_r].hh.b0=2 then begin r:=mem[prev_r].hh.rh;
if r=2993 then begin cur_active_width[1]:=cur_active_width[1]-mem[prev_r
+1].int;cur_active_width[2]:=cur_active_width[2]-mem[prev_r+2].int;
...
mem[prev_prev_r].hh.rh:=2993;free_node(prev_r,7);prev_r:=prev_prev_r;
...
10:{858:}if cur_p=printed_node then if cur_p<>0 then if mem[cur_p].hh.b0
=7 then begin t:=mem[cur_p].hh.b1;while t>0 do begin t:=t-1;
...
if eqtb[2889].hh.rh<>1 then begin r:=new_param_glue(7);mem[r].hh.rh:=q;
q:=r;end{:887};
...
adjust_tail:=2995;just_box:=hpack(q,cur_width,0);
mem[just_box+4].int:=cur_indent{:889};{888:}append_to_vlist(just_box);
if 2995<>adjust_tail then begin mem[cur_list.tail_field].hh.rh:=mem[2995
].hh.rh;cur_list.tail_field:=adjust_tail;end;adjust_tail:=0{:888};
...
if cur_p<>0 then if not post_disc_break then{879:}begin r:=2997;
...
31:if r<>2997 then begin mem[r].hh.rh:=0;
flush_node_list(mem[2997].hh.rh);mem[2997].hh.rh:=q;end;end{:879};
until cur_p=0;
if(cur_line<>best_line)or(mem[2997].hh.rh<>0)then confusion(939);
...
k:font_index;begin hyphen_passed:=0;t:=2996;w:=0;mem[2996].hh.rh:=0;
...
if hyphen_passed=0 then begin mem[s].hh.rh:=mem[2996].hh.rh;
while mem[s].hh.rh>0 do s:=mem[s].hh.rh;if odd(hyf[j-1])then begin l:=j;
hyphen_passed:=j-1;mem[2996].hh.rh:=0;end;end;
if hyphen_passed>0 then{914:}repeat r:=get_node(2);
mem[r].hh.rh:=mem[2996].hh.rh;mem[r].hh.b0:=7;major_tail:=r;r_count:=0;
...
if mem[2996].hh.rh>0 then begin if minor_tail=0 then mem[r+1].hh.lh:=mem
[2996].hh.rh else mem[minor_tail].hh.rh:=mem[2996].hh.rh;
minor_tail:=mem[2996].hh.rh;
...
if mem[2996].hh.rh>0 then begin if minor_tail=0 then mem[r+1].hh.rh:=mem
[2996].hh.rh else mem[minor_tail].hh.rh:=mem[2996].hh.rh;
minor_tail:=mem[2996].hh.rh;
while mem[minor_tail].hh.rh>0 do minor_tail:=mem[minor_tail].hh.rh;end;
until l>=j;while l>j do{917:}begin j:=reconstitute(j,hn,bchar,256)+1;
mem[major_tail].hh.rh:=mem[2996].hh.rh;
while mem[major_tail].hh.rh>0 do begin major_tail:=mem[major_tail].hh.rh
...
mem[2996].hh.rh:=0;until not odd(hyf[j-1]){:914};until j>hn;
...
print_nl(262);print(957);end;begin help_ptr:=1;help_line[0]:=956;end;
...
mem[2988].hh.rh:=scan_toks(false,false);flush_list(def_ref);end;end;
...
{816:}mem[2997].hh.rh:=mem[cur_list.head_field].hh.rh;
...
begin_diagnostic;print_nl(933);end;second_pass:=false;final_pass:=false;
end else begin threshold:=eqtb[5264].int;second_pass:=true;
...
{864:}q:=get_node(3);mem[q].hh.b0:=0;mem[q].hh.b1:=2;mem[q].hh.rh:=2993;
mem[q+1].hh.rh:=0;mem[q+1].hh.lh:=cur_list.pg_field+1;mem[q+2].int:=0;
mem[2993].hh.rh:=q;active_width[1]:=background[1];
active_width[2]:=background[2];active_width[3]:=background[3];
active_width[4]:=background[4];active_width[5]:=background[5];
active_width[6]:=background[6];passive:=0;printed_node:=2997;
pass_number:=0;font_in_short_display:=0{:864};cur_p:=mem[2997].hh.rh;
auto_breaking:=true;prev_p:=cur_p;
while(cur_p<>0)and(mem[2993].hh.rh<>2993)do{866:}begin if(cur_p>=
...
if mem[2993].hh.rh<>2993 then begin{874:}r:=mem[2993].hh.rh;
...
until r=2993;best_line:=mem[best_bet+1].hh.lh{:874};
if eqtb[5282].int=0 then goto 30;{875:}begin r:=mem[2993].hh.rh;
actual_looseness:=0;
...
r:=mem[r].hh.rh;until r=2993;best_line:=mem[best_bet+1].hh.lh;end{:875};
if(actual_looseness=eqtb[5282].int)or final_pass then goto 30;end;
end{:873};{865:}q:=mem[2993].hh.rh;
while q<>2993 do begin cur_p:=mem[q].hh.rh;
...
q:=cur_p;end{:865};
if not second_pass then begin if eqtb[5295].int>0 then print_nl(934);
threshold:=eqtb[5264].int;second_pass:=true;
final_pass:=(eqtb[5850].int<=0);
end else begin if eqtb[5295].int>0 then print_nl(935);
...
{865:}q:=mem[2993].hh.rh;while q<>2993 do begin cur_p:=mem[q].hh.rh;
if mem[q].hh.b0=2 then free_node(q,7)else free_node(q,3);q:=cur_p;end;
q:=passive;while q<>0 do begin cur_p:=mem[q].hh.rh;free_node(q,2);
...
s:=make_string;{940:}if hyph_count=307 then overflow(948,307);
...
var prev_p:halfword;q:halfword;begin prev_p:=2997;mem[2997].hh.rh:=p;
...
end;others:confusion(959)end;prune_page_top:=mem[2997].hh.rh;end;
...
if eqtb[5316].int<=0 then{1018:}begin r:=mem[3000].hh.rh;
while r<>3000 do begin if mem[r+2].hh.lh<>0 then begin n:=mem[r].hh.b1-0
;ensure_vbox(n);
if eqtb[3678+n].hh.rh=0 then eqtb[3678+n].hh.rh:=new_null_box;
p:=eqtb[3678+n].hh.rh+5;while mem[p].hh.rh<>0 do p:=mem[p].hh.rh;
mem[r+2].hh.rh:=p;end;r:=mem[r].hh.rh;end;end{:1018};q:=2996;
mem[q].hh.rh:=0;prev_p:=2998;p:=mem[prev_p].hh.rh;
while p<>best_page_break do begin if mem[p].hh.b0=3 then begin if eqtb[
5316].int<=0 then{1020:}begin r:=mem[3000].hh.rh;
...
begin begin if interaction=3 then;print_nl(262);print(1005);end;
...
{1023:}begin if mem[2998].hh.rh<>0 then begin if mem[2999].hh.rh=0 then
if nest_ptr=0 then cur_list.tail_field:=page_tail else nest[0].
tail_field:=page_tail else mem[page_tail].hh.rh:=mem[2999].hh.rh;
mem[2999].hh.rh:=mem[2998].hh.rh;mem[2998].hh.rh:=0;page_tail:=2998;end;
ship_out(eqtb[3933].hh.rh);eqtb[3933].hh.rh:=0;end{:1023};10:end;
...
begin if(mem[2999].hh.rh=0)or output_active then goto 10;
repeat 22:p:=mem[2999].hh.rh;
...
mem[q].hh.rh:=p;mem[2999].hh.rh:=q;goto 22;
...
if eqtb[5296].int>0 then{1011:}begin begin_diagnostic;print_nl(1001);
...
best_size:=page_so_far[0];least_page_cost:=c;r:=mem[3000].hh.rh;
...
mem[2999].hh.rh:=mem[p].hh.rh;mem[p].hh.rh:=0;goto 30{:998};
31:{999:}mem[2999].hh.rh:=mem[p].hh.rh;mem[p].hh.rh:=0;
flush_node_list(p){:999};30:{:997};until mem[2999].hh.rh=0;
{995:}if nest_ptr=0 then cur_list.tail_field:=2999 else nest[0].
tail_field:=2999{:995};10:end;{:994}{1030:}{1043:}procedure app_space;
var q:halfword;
begin if(cur_list.aux_field.hh.lh>=2000)and(eqtb[2895].hh.rh<>1)then q:=
new_param_glue(13)else begin if eqtb[2894].hh.rh<>1 then main_p:=eqtb[
2894].hh.rh else{1042:}begin main_p:=font_glue[eqtb[3934].hh.rh];
if main_p=0 then begin main_p:=new_spec(1);
...
begin if privileged then begin if(2998=page_tail)and(cur_list.head_field
=cur_list.tail_field)and(dead_cycles=0)then begin its_all_over:=true;
goto 10;end;back_input;
begin mem[cur_list.tail_field].hh.rh:=new_null_box;
cur_list.tail_field:=mem[cur_list.tail_field].hh.rh;end;
mem[cur_list.tail_field+1].int:=eqtb[5833].int;
begin mem[cur_list.tail_field].hh.rh:=new_glue(9);
cur_list.tail_field:=mem[cur_list.tail_field].hh.rh;end;
...
var s:small_number;begin s:=cur_chr;case s of 0:cur_val:=5;1:cur_val:=9;
2:cur_val:=13;3:cur_val:=17;4:scan_glue(2);5:scan_glue(3);end;
...
end{:1066}else begin back_input;p:=get_avail;mem[2997].hh.rh:=p;
...
begin_token_list(mem[2997].hh.rh,4);begin help_ptr:=5;
...
if adjust_tail<>0 then begin if 2995<>adjust_tail then begin mem[
cur_list.tail_field].hh.rh:=mem[2995].hh.rh;
...
goto 40;end;6:{652:}begin mem[2988]:=mem[p+1];
mem[2988].hh.rh:=mem[p].hh.rh;p:=2988;goto 21;end{:652};
...
scan_delimiter(2988,false);scan_delimiter(2988,false);end;
...
scan_delimiter(2988,false);begin if interaction=3 then;print_nl(262);
...
interaction=3 then;print_nl(262);print(1162);end;begin help_ptr:=3;
...
mlist_penalties:=false;mlist_to_hlist;a:=hpack(mem[2997].hh.rh,0,1);
...
mlist_to_hlist;mem[cur_list.tail_field].hh.rh:=mem[2997].hh.rh;
while mem[cur_list.tail_field].hh.rh<>0 do cur_list.tail_field:=mem[
...
mlist_penalties:=false;mlist_to_hlist;p:=mem[2997].hh.rh;
adjust_tail:=2995;b:=hpack(p,0,1);p:=mem[b+5].hh.rh;t:=adjust_tail;
...
if t<>2995 then begin mem[cur_list.tail_field].hh.rh:=mem[2995].hh.rh;
...
help_line[0]:=1189;end;if cur_cs=0 then back_input;cur_tok:=6709;
...
].int=0)then begin mem[1].hh.rh:=mem[1].hh.rh+1;
delete_glue_ref(cur_val);cur_val:=1;end;end;
...
then;print_nl(262);print(1202);end;print_int(cur_val);
if p<5574 then print(1203)else print(1204);print_int(n);
...
s:str_number;begin c:=cur_chr;mem[2988].hh.rh:=scan_toks(false,true);
...
begin mem[def_ref].hh.rh:=avail;avail:=def_ref;dyn_used:=dyn_used-1;end;
end;{:1288}{1293:}procedure show_whatever;label 50;var p:halfword;
...
if interaction=3 then;print_nl(1248);token_show(2997);
flush_list(mem[2997].hh.rh);goto 50;end{:1297}end;
...
put(fmt_file);end;begin fmt_file^.int:=1;put(fmt_file);end;
begin fmt_file^.int:=3000;put(fmt_file);end;begin fmt_file^.int:=6106;
...
put(fmt_file);end;begin fmt_file^.int:=rover;put(fmt_file);end;p:=1;
...
print_int(cs_count);print(1262){:1318}{:1313};
...
print_nl(1265);print_esc(hash[2624+k].rh);print_char(61);
...
3:begin adjust_tail:=2995;package(0);end;4:begin end_graf;package(0);
...
tail_field:=page_tail;mem[page_tail].hh.rh:=mem[2999].hh.rh;
mem[2999].hh.rh:=mem[2998].hh.rh;mem[2998].hh.rh:=0;page_tail:=2998;end;
pop_nest;build_page;end{:1026};{:1100}{1118:}10:build_discretionary;
{:1118}{1132:}6:begin back_input;cur_tok:=6710;
begin if interaction=3 then;print_nl(262);print(625);end;print_esc(899);
print(626);begin help_ptr:=1;help_line[0]:=1125;end;ins_error;end;
...
258:begin begin mem[cur_list.tail_field].hh.rh:=new_glue(1);
cur_list.tail_field:=mem[cur_list.tail_field].hh.rh;end;
...
begin mem[lig_stack].hh.rh:=avail;avail:=lig_stack;dyn_used:=dyn_used-1;
end;goto 60;end;mem[cur_list.tail_field].hh.rh:=lig_stack;
...
120:{1041:}if eqtb[2894].hh.rh=1 then begin{1042:}begin main_p:=
font_glue[eqtb[3934].hh.rh];if main_p=0 then begin main_p:=new_spec(1);
main_k:=param_base[eqtb[3934].hh.rh]+2;
mem[main_p+1].int:=font_info[main_k].int;
mem[main_p+2].int:=font_info[main_k+1].int;
mem[main_p+3].int:=font_info[main_k+2].int;
...
begin get(fmt_file);x:=fmt_file^.int;end;if x<>1 then goto 6666;
begin get(fmt_file);x:=fmt_file^.int;end;if x<>3000 then goto 6666;
...
x:=fmt_file^.int;end;
if(x<1020)or(x>2986)then goto 6666 else lo_mem_max:=x;end;
begin begin get(fmt_file);x:=fmt_file^.int;end;
if(x<21)or(x>lo_mem_max)then goto 6666 else rover:=x;end;p:=1;q:=rover;
...
if mem_min<-1 then begin p:=mem[rover+1].hh.lh;q:=mem_min+1;
mem[mem_min].hh.rh:=0;mem[mem_min].hh.lh:=0;mem[p+1].hh.rh:=q;
mem[rover+1].hh.lh:=q;mem[q+1].hh.rh:=rover;mem[q+1].hh.lh:=p;
mem[q].hh.rh:=65535;mem[q].hh.lh:=1-q;end;begin begin get(fmt_file);
x:=fmt_file^.int;end;
if(x<lo_mem_max+1)or(x>2987)then goto 6666 else hi_mem_min:=x;end;
begin begin get(fmt_file);x:=fmt_file^.int;end;
if(x<0)or(x>3000)then goto 6666 else avail:=x;end;mem_end:=3000;
...
begin c:=cur_chr;if c<>1 then eqtb[5312].int:=-1;
if job_name=0 then open_log_file;
...
eqtb[2618]:=eqtb[cur_val];primitive(775,106,4);primitive(776,106,3);
{:491}{553:}primitive(801,87,0);hash[2624].rh:=801;
eqtb[2624]:=eqtb[cur_val];{:553}{780:}primitive(898,4,256);
primitive(899,5,257);hash[2615].rh:=899;eqtb[2615]:=eqtb[cur_val];
primitive(900,5,258);hash[2619].rh:=901;hash[2620].rh:=901;
eqtb[2620].hh.b0:=9;eqtb[2620].hh.rh:=2989;eqtb[2620].hh.b1:=1;
...
primitive(877,49,31);hash[2617].rh:=877;eqtb[2617]:=eqtb[cur_val];
...
if max_in_open>=128 then bad:=6;if 3000<267 then bad:=7;
{:14}{111:}if(mem_min<>1)or(mem_max<>3000)then bad:=10;
if(mem_min>1)or(mem_max<3000)then bad:=10;
if(0>0)or(255<127)then bad:=11;if(0>0)or(65535<32767)then bad:=12;
if(0<0)or(255>65535)then bad:=13;
if(mem_min<0)or(mem_max>=65535)or(1-mem_min>65536)then bad:=14;
...
{:522}{1249:}if 2*65535<3000-mem_min then bad:=41;
...
if(eqtb[5311].int<0)or(eqtb[5311].int>255)then cur_input.limit_field:=
cur_input.limit_field-1 else buffer[cur_input.limit_field]:=eqtb[5311].
int;fix_date_and_time;{765:}magic_offset:=str_start[892]-9*16{:765};
{75:}if interaction=0 then selector:=16 else selector:=17{:75};
