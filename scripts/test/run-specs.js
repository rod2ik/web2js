...
function runTest(filename) {
    ...
    if (!fs.existsSync(path.join(projectRoot, 'build/parser.js')))
        child_process.execSync('yarn build:parser', { cwd: projectRoot, stdio: 'inherit' });
    ...
    let output;
    try {
        output = child_process.execSync(`node ../src/compiler/index.js ${filename} 2> /dev/null`).toString();
